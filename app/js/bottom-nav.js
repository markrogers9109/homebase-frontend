  ...
  var methods = {
    init : function(options) {
      ...
      $(this).each(function(){
        ...
        if (options.edge == 'top') {
          menu_id.css('transform', 'translateY(-100%)');
        ...
        var removeMenu = function(restoreNav) {
          ...
          if (options.edge === 'top') {
            // Reset phantom div
            $dragTarget.css({width: '', right: '', top: '0'});
            menu_id.velocity(
              {'translateY': '-100%'},
              { duration: 200,
                queue: false,
            ...
            // Reset phantom div
            $dragTarget.css({width: '', right: '0', top: ''});
            menu_id.velocity(
              {'translateY': '100%'},
              { duration: 200,
                queue: false,
        ...
        if (options.draggable) {
          ...
          $dragTarget.hammer({
            ...
            if (e.gesture.pointerType == "touch") {
              ...
              var velocityX = e.gesture.velocityX;
              var x = e.gesture.center.x;
              var topPos = x - options.menuWidth;
              var rightPos = x - options.menuWidth / 2;
              if (topPos > 0 ) {
                topPos = 0;
              }
              ...
              if (options.edge === 'top') {
                ...
                if ((menuOut && velocityX <= 0.3) || velocityX < -0.5) {
                  // Return menu to open
                  if (topPos !== 0) {
                    menu_id.velocity({'translateY': [0, topPos]}, {duration: 300, queue: false, easing: 'easeOutQuad'});
                  }

                  $overlay.velocity({opacity: 1 }, {duration: 50, queue: false, easing: 'easeOutQuad'});
                  $dragTarget.css({width: '50%', right: 0, top: ''});
                  menuOut = true;
                }
                  ...
                  // Slide menu closed
                  menu_id.velocity({'translateY': [-1 * options.menuWidth - 10, topPos]}, {duration: 200, queue: false, easing: 'easeOutQuad'});
                  $overlay.velocity({opacity: 0 }, {duration: 200, queue: false, easing: 'easeOutQuad',
                    complete: function () {
                      $(this).remove();
                    }});
                  $dragTarget.css({width: '10px', right: '', top: 0});
                }
              }
              else {
                if ((menuOut && velocityX >= -0.3) || velocityX > 0.5) {
                  ...
                  $overlay.velocity({opacity: 1 }, {duration: 50, queue: false, easing: 'easeOutQuad'});
                  $dragTarget.css({width: '50%', right: '', top: 0});
                  menuOut = true;
                }
                else if (!menuOut || velocityX < -0.3) {
                  // Enable Scrolling
                  ...
                  $dragTarget.css({width: '10px', right: 0, top: ''});
                }
              }
